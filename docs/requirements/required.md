...
#### 3.2 候选人面试流程  
- 功能 F3：候选人选择面试场次／进入面试界面  
  - 用户故事：作为候选人，我希望能看到可参加的面试场次并进入。  
- 功能 F4：编程题模块：题目展示 + 代码编辑器 + 提交 + 测试执行  
  - 用户故事：作为候选人，我希望在系统中编写代码并提交之后看到运行结果。  
- 功能 F5：问答题模块：文字输入方式（可后续拓展语音） + 提交  
  - 用户故事：作为候选人，我希望回答行为/系统设计题并提交。  
- 功能 F6：自动评分流程：系统对编程题执行测试、分析得分；系统对问答题调用 AI 模型生成初步评分建议  
  - 用户故事：作为候选人，我希望提交后系统可以快速给出反馈。  
- 功能 F7：候选人查看结果界面：得分、报告摘要（初步）  
  - 用户故事：作为候选人，我希望面试结束后能看到我的得分与简要反馈。  

#### 3.3 面试官后台流程  
- 功能 F8：面试官查看候选人提交记录：代码、运行日志、问答内容  
- 功能 F9：面试官查看系统生成的 AI 评分建议，并可手动调整评分  
- 功能 F10：面试官确认最终评分并生成候选人最终报告  
  - 用户故事：作为面试官，我希望快速查看候选人表现并作出判断。  

#### 3.4 管理员与题库管理  
- 功能 F11：管理员／题库管理员登录后台，新增／编辑／删除题目（编程题与问答题）  
- 功能 F12：管理员设定面试模板：题目组合、时间限制、语言选项、难度等级  
- 功能 F13：系统日志及用户管理：查看用户注册／登录情况、角色分配、权限管理  

#### 3.5 报告与统计分析  
- 功能 F14：生成候选人完整面试报告：技术得分、行为得分、综合建议  
- 功能 F15：统计仪表盘（面试官或管理员视角）：今日/本周面试数、平均得分、题目平均得分、通过率  
...
### 6. 优先级与里程碑  
- Must Have（优先上线）： F1-F7（用户登录、候选人流程、编程/问答题、自动评分）  
- Should Have（次优）： F8-F10（面试官后台）  
- Could Have（可后期迭代）： F11-F15（题库管理、统计仪表盘）  
